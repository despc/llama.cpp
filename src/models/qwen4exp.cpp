...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    // Tell attention how many finite entries each mask row has.  The backend uses it
    // to compact the mask into an index list and visit only the selected positions;
    // where it cannot, it ignores the number and the dense path is unchanged.
