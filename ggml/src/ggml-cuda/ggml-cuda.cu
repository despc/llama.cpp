...
static bool ggml_backend_cuda_comm_init_mixed(ggml_backend_cuda_comm_context * ret) {
    ...
    // The grid every rank runs.  Measured: a larger grid is worth nothing to this
    // collective and costs 31% with the grid-wide barrier the reduce-scatter
    // uses, because every phase already saturates the link in one direction.
    // Default 8; the knob stays because each runtime now refuses a grid its
    // devices cannot hold resident, and that check is what makes the barrier
    ...
    ret->mixed_host = ggml_aligned_malloc(shared_bytes);
    if (!ret->mixed_host) {
        return false;
    }
    memset(ret->mixed_host, 0, shared_bytes);
...
static void ggml_cuda_mul_mat_cublas(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (ggml_is_quantized(compute_type)) {
        ...
        compute_type = GGML_TYPE_F32;
               ...
               !ggml_env_flag_enabled("GGML_CUDA_KEEP_BF16")) {
        // Upstream moves BF16 matmuls to F32 on devices without BF16 hardware, which
        // on this machine means the Teslas.  It changes arithmetic wherever a model
        // carries BF16 weights -- Flash-Next's QSA indexer projections are BF16, and
        // this path is why its output differs from the pre-merge build.
        // GGML_CUDA_KEEP_BF16=1 keeps the old compute type so that can be re-checked.
