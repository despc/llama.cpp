...
// every peer's contribution across its own link: N out and 3N in at four ranks.
// All communication here goes through mapped host memory -- these cards have no
// P2P -- so those bytes cross PCIe twice and the profile puts about 90% of a 27B
// prefill in the collective rather than in compute.
//
// Here each rank reduces one shard and then everyone exchanges finished shards:
// N + N/4 out, 3N/4 + 3N/4 in.  That is 2.75N against 4N overall and, in the
// direction that dominates, 1.5N of reads against 3N.
//
// It distributes the reduction rather than concentrating it, which is what
// separates it from republishing to a leader: nobody idles while one rank works.
//
// Arithmetic is untouched.  Every output element is reduced exactly once, by the
// rank that owns its shard, with the same ggml_cuda_mixed_ar_reduce over the same
// values in the same order, so the result is bit-identical to the single-shot
// kernel and to the meta backend.
template <typename T>
static __global__ void ggml_cuda_mixed_ar_rs_kernel(
        const T *                   sendbuf,
        T *                         recvbuf,
        T * __restrict__            slot_data,
        int                         rank,
...
#pragma unroll
    ...
    auto barrier = [&](int word) {
        ...
        __syncthreads();
        if (tid == 0) {
            *(volatile uint32_t *) (my_sig + word) = token;
            __threadfence_system();
            // Grid-wide, across every rank: the phases stripe by global thread, so
            // the region this block reads next was written by whichever blocks
            ...
            for (int peer = 0; peer < n_ranks; ++peer) {
                for (int b = 0; b < (int) gridDim.x; ++b) {
                    ...
                    const uint32_t * ps = arrival_slot +
                        ((size_t) peer * GGML_CUDA_MIXED_AR_BLOCKS + b) * SIGNAL_INTS + word;
    ...
    //    region of its own shard -- and this rank takes its value from sendbuf.
...
#pragma unroll
    ...
    for (int peer = 0; peer < n_ranks; ++peer) {
        ...
        const T * host_peer = slot_data + (size_t) peer * rank_stride;
        for (int i = shard_lo(peer) + gtid; i < shard_lo(peer + 1); i += gnt) {
            const int off = i * ELEMS_PER_VEC;
            T got[ELEMS_PER_VEC];
...
// A meeting point for every rank, one block wide.
//
// Verification only, to keep the collective under test apart from the reference
// it is compared against: without it a rank could start publishing its second
// pass into a slot a peer is still reading for its first.  One block is always
// resident, and it polls one word per peer.
static __global__ void ggml_cuda_mixed_ar_meet_kernel(
        uint32_t * arrival_slot, int rank, int n_ranks, uint32_t token, int word) {
    if (threadIdx.x != 0) {
        return;
    }
    constexpr int SIGNAL_INTS = (int) (GGML_CUDA_MIXED_AR_SIGNAL_STRIDE / sizeof(uint32_t));
    // The previous kernel's stores landed when it completed; this orders the flag
    // that announces them after them.
    __threadfence_system();
    uint32_t * my = arrival_slot + ((size_t) rank * GGML_CUDA_MIXED_AR_BLOCKS) * SIGNAL_INTS;
    *(volatile uint32_t *) (my + word) = token;
    __threadfence_system();
    for (int p = 0; p < n_ranks; ++p) {
        ...
        while (*(const volatile uint32_t *) (ps + word) != token) {
#if __CUDA_ARCH__ >= GGML_CUDA_CC_VOLTA
            __nanosleep(100);
#else
            NO_DEVICE_CODE;
...
void ggml_cuda_mixed_ar_group_free(void * context) {
    ...
    if (group->host_registered) {
        ggml_cuda_set_device(group->devices[0]);
        CUDA_CHECK(cudaHostUnregister(group->shared_host));
    }
    delete group;
}
...
void * ggml_cuda_mixed_ar_group_init(const ggml_cuda_mixed_ar_group_config * config) {
    if (!config || config->abi_version != GGML_CUDA_MIXED_AR_ABI_VERSION ||
        ...
        config->n_ranks < 2 || config->n_ranks > GGML_CUDA_MAX_DEVICES ||
    ...
    if (!ggml_backend_is_cuda(config->backends[0])) {
        return nullptr;
    }
    ...
    for (size_t i = 0; i < config->n_backends; ++i) {
        const int rank = config->ranks[i];
        if (rank < 0 || (size_t) rank >= config->n_ranks ||
            !ggml_backend_is_cuda(config->backends[i]) ||
            ggml_backend_dev_backend_reg(ggml_backend_get_device(config->backends[i])) != registry) {
            ggml_cuda_mixed_ar_group_free(group);
            return nullptr;
        }
        ...
        if (!GGML_CUDA_CC_IS_NVIDIA(cc) || cc < GGML_CUDA_CC_VOLTA ||
            std::find(group->devices.begin(), group->devices.end(), cuda_ctx->device) != group->devices.end() ||
            std::find(group->ranks.begin(), group->ranks.begin() + i, rank) != group->ranks.begin() + i) {
            ggml_cuda_mixed_ar_group_free(group);
            return nullptr;
        }
    ...
    if (rc != cudaSuccess) {
        GGML_LOG_WARN("%s: cudaHostRegister failed: %s\n", __func__, cudaGetErrorString(rc));
...
static void ggml_cuda_mixed_ar_launch(
    ...
    auto meet = [&](int word) {
        ggml_cuda_mixed_ar_meet_kernel<<<1, 32, 0, stream>>>(
            arrival_slot, rank, n_ranks, token, word);
    };
...
bool ggml_cuda_mixed_ar_group_enqueue(
    ...
    // Validate the entire group before launching any rank.
    for (int rank : group->ranks) {
        const ggml_tensor * tensor = tensors[rank];
        if (!tensor || !tensor->data || !ggml_is_contiguously_allocated(tensor) ||
            (tensor->type != GGML_TYPE_F32 && tensor->type != GGML_TYPE_F16 && tensor->type != GGML_TYPE_BF16) ||
            ggml_nelements(tensor) <= 0 || ggml_nbytes(tensor) > GGML_CUDA_MIXED_AR_RANK_BYTES) {
            return false;
        }
    }
...
bool ggml_cuda_mixed_ar_group_enqueue(void *, ggml_tensor **, size_t, uint32_t) {
    return false;
}
