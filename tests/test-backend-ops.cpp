...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // qwen4exp prefills with hundreds of query tokens per call, so the sparse path
    // has to hold at every batch size, not only the single-token one.
    for (int nb : {2, 4, 8, 16, 32, 64, 128, 512}) {
        test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 1}, 8192, nb, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, {0, 1, 2, 3}, true, false, 2051));
    }
